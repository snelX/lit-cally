...
export function safeFrom<T extends PlainDate | PlainYearMonth>(
...
export function parseDateRangeProp(value: string | undefined): [PlainDate, PlainDate] | [] {
  if (!value) return [];
...
export function parseDateMultiProp(value: string | undefined): PlainDate[] {
  if (!value) return [];
  const result = [];
...
export function createDateFormatter(options: DateFormatOptions, locale?: string) {
